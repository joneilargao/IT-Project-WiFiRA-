...
$query = $pdo->prepare("SELECT voucherCode, voucherType, voucherAmount, datePrinted FROM vouchers WHERE WEEK(datePrinted)=WEEK(CURRENT_DATE()) AND MONTH(datePrinted)=MONTH(CURRENT_DATE()) AND YEAR(datePrinted)=YEAR(CURRENT_DATE()) AND voucherStatus='sold' ORDER BY datePrinted DESC");
$query->execute();
