...
$query = $pdo->prepare("SELECT  vouchers.kioskId as kioskId, kioskName AS kioskName, COUNT(voucherCode) AS totalVoucherCode, 
        SUM(voucherAmount) AS totalAmount FROM vouchers INNER JOIN kioskmachine ON kioskmachine.kioskId = vouchers.kioskId WHERE YEAR(vouchers.dateSold)=YEAR(CURRENT_DATE()) and vouchers.voucherStatus = 'Sold' GROUP BY kioskId ASC;");
$query->execute();
...
$query1 = $pdo->prepare("SELECT COUNT(voucherCode) as totalVoucherCode, SUM(voucherAmount) as totalAmount FROM vouchers  WHERE YEAR(vouchers.dateSold)=YEAR(CURRENT_DATE()) and voucherStatus='Sold' and kioskId is not null");
$query1->execute();
