...
$query = $pdo->prepare("SELECT voucherCode, voucherType, voucherAmount, datePrinted FROM vouchers WHERE MONTH(datePrinted)=MONTH(CURRENT_DATE()) AND YEAR(datePrinted)=YEAR(CURRENT_DATE()) AND voucherStatus='sold' AND accountNo='$user_id' ORDER BY datePrinted DESC");
$query->execute();
