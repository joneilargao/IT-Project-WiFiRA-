...
$query = $pdo->prepare("SELECT voucherCode, voucherType, voucherAmount, dateSold FROM vouchers WHERE MONTH(dateSold)=MONTH(CURRENT_DATE()) AND YEAR(dateSold)=YEAR(CURRENT_DATE()) AND voucherStatus='sold' AND accountNo='$user_id' ORDER BY dateSold DESC");
$query->execute();
